...
Graph::Graph(std::string & airportsFile, std::string & routesFile){
    insertAllVertices(airportsFile);
...
void Graph::insertAllEdge(std::string & filesName){
    std::fstream file;
    //open the file
    file.open(filesName, ios::in);
    if (file.is_open()){   
...
void Graph::adjMatrix(PageRank *pr_obj){
    ...
    //populate the namelist of pagerank object
    int x = 0;
